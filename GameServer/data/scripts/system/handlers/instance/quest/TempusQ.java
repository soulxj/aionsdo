...
public class TempusQ extends GeneralInstanceHandler
{
	...
	public void handleUseItemFinish(final Player player, final Npc npc) {
		...
		switch (npc.getNpcId()) {
			...
			case 800507: //idf4re_solo_teleport_01.
			    tempus_stagestart_a(player, 412.0000f, 458.0000f, 114.0000f, (byte) 30);
			break;
			case 800508: //idf4re_solo_teleport_02.
			    tempus_stagestart_b(player, 457.0000f, 457.0000f, 116.0000f, (byte) 30);
			break;
			case 800509: //idf4re_solo_teleport_03.
				tempus_stagestart_c(player, 498.0000f, 457.0000f, 114.0000f, (byte) 30);
			break;
			...
			case 800583: //idf4re_solo_evileye_switch_03.
			    if (player.getInventory().decreaseByItemId(185000123, 1)) {
				    //to do...
				}
			break;
			...
			break;
			case 800636: //idf4re_solo_teleport_07.
			    tempus_returnstart_a(player, 465.0000f, 441.0000f, 114.0000f, (byte) 0);
			break;
			case 800637: //idf4re_solo_teleport_08.
			    tempus_returnstart_b(player, 465.0000f, 441.0000f, 132.0000f, (byte) 0);
			break;
			case 800638: //idf4re_solo_teleport_09.
				tempus_returnstart_c(player, 465.0000f, 441.0000f, 149.0000f, (byte) 0);
			break;
		} if (instanceReward.getInstanceScoreType().isStartProgress()) {
			instanceReward.addPoints(score);
			sendPacket(npc.getObjectTemplate().getNameId(), score);
	...
	protected void tempus_stagestart_a(Player player, float x, float y, float z, byte h) {
		TeleportService2.teleportTo(player, mapId, instanceId, x, y, z, h);
	}
	protected void tempus_stagestart_b(Player player, float x, float y, float z, byte h) {
		TeleportService2.teleportTo(player, mapId, instanceId, x, y, z, h);
	}
	protected void tempus_stagestart_c(Player player, float x, float y, float z, byte h) {
		TeleportService2.teleportTo(player, mapId, instanceId, x, y, z, h);
	}
	protected void tempus_returnstart_a(Player player, float x, float y, float z, byte h) {
		TeleportService2.teleportTo(player, mapId, instanceId, x, y, z, h);
	}
	protected void tempus_returnstart_b(Player player, float x, float y, float z, byte h) {
		TeleportService2.teleportTo(player, mapId, instanceId, x, y, z, h);
	}
	protected void tempus_returnstart_c(Player player, float x, float y, float z, byte h) {
		TeleportService2.teleportTo(player, mapId, instanceId, x, y, z, h);
	}
	...
	private int checkRank(int totalPoints) {
		if (totalPoints >= 12500) { //Rank S.
			...
			rank = 4;
		} else if (totalPoints >= 6656) { //Rank D.
			rank = 5;
