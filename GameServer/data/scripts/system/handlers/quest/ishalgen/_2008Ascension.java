...
public class _2008Ascension extends QuestHandler
{
				...
				switch (env.getDialog()) {
					case START_DIALOG:
						if (var == 99) {
							...
							PacketSendUtility.sendPacket(player, new S_ACTION(player, EmotionType.START_FLYTELEPORT, 3001, 0));
							qs.setQuestVar(50);
							updateQuestStatus(env);
	...
	public boolean onDieEvent(QuestEnv env) {
		...
		if (var == 5 || (var == 6 && player.getPlayerClass().isStartingClass()) || (var >= 51 && var <= 53)) {
			...
			updateQuestStatus(env);
			PacketSendUtility
					.sendPacket(player, new S_MESSAGE_CODE(SystemMessageId.QUEST_FAILED_$1, DataManager.QUEST_DATA.getQuestById(questId).getName()));
		}
		return false;
	}
}