...
public class MySQL5LegionDAO extends LegionDAO {
    ...
    private static final String UPDATE_LEGION_QUERY = "UPDATE legions SET name=?, level=?, contribution_points=?, deputy_permission=?, centurion_permission=?, legionary_permission=?, volunteer_permission=?, disband_time=?, " +
			"description=?, joinType=?, minJoinLevel=?, mainActivity=?, growthLevel=?, growthExp=?, essenceCoin=?, essenceFragment=?, productivityStep=? WHERE id=?";
	...
	public void storeLegion(final Legion legion) {
		DB.insertUpdate(UPDATE_LEGION_QUERY, new IUStH() {
			...
			public void handleInsertUpdate(PreparedStatement stmt) throws SQLException {
				...
				stmt.setInt(12, legion.getActivity());
				stmt.setInt(13, legion.getGrowthLevel());
				stmt.setLong(14, legion.getGrowthExp());
				stmt.setLong(15, legion.getEssenceCoin());
				stmt.setLong(16, legion.getEssenceFragment());
				stmt.setLong(17, legion.getProductivityStep());
				stmt.setInt(18, legion.getLegionId());
				stmt.execute();
			}
		});
	}
		...
		boolean success = DB.select(SELECT_LEGION_QUERY2, new ParamReadStH() {
			...
			public void handleRead(ResultSet resultSet) throws SQLException {
				while (resultSet.next()) {
					...
					legion.setActivity(resultSet.getInt("mainActivity"));
					legion.setGrowthLevel(resultSet.getInt("growthLevel"));
					legion.setGrowthExp(resultSet.getLong("growthExp"));
					legion.setEssenceCoin(resultSet.getLong("essenceCoin"));
					legion.setEssenceFragment(resultSet.getLong("essenceFragment"));
					legion.setProductivityStep(resultSet.getInt("productivityStep"));
				}
			}
		});
