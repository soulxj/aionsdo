...
public class S_GUILD_INFO extends AionServerPacket
{
	...
	protected void writeImpl(AionConnection con) {
		...
		if(GSConfig.SERVER_COUNTRY_CODE == 0 || GSConfig.SERVER_COUNTRY_CODE == 5) {
			writeS(legion.getLegionDescription()); //join message
			writeC(legion.getLegionJoinType()); //join type
			writeH(legion.getMinLevel()); //min join level
			writeH(legion.getActivity()); //main activity
			writeD(legion.getGrowthLevel()); //Growth Level
			writeQ(legion.getGrowthExp()); //growth Exp
			writeQ(legion.getEssenceCoin());//legion essence coin
			writeQ(legion.getEssenceFragment());//legion essence Fragment
			writeD(legion.getProductivityStep());//legion productivity step
		}
	}
}