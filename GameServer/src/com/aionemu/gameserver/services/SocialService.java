...
public class SocialService
{
	...
	public static boolean deleteBlockedUser(Player player, int blockedUserId) {
		if (DAOManager.getDAO(BlockListDAO.class).delBlockedUser(player.getObjectId(), blockedUserId)) {
			...
			player.getClientConnection().sendPacket(new S_BLOCK_RESULT(S_BLOCK_RESULT.UNBLOCK_SUCCESSFUL, DAOManager.getDAO(PlayerDAO.class).loadPlayerCommonData(blockedUserId).getName()));
			player.getClientConnection().sendPacket(new S_BLOCK_LIST());
			return true;
		}
		return false;
