...
public class ClassChangeService
{
		...
		if (CustomConfig.ENABLE_SIMPLE_2NDCLASS) {
			if (playerRace == Race.ELYOS) {
				...
				if (player.havePermission(MembershipConfig.STIGMA_SLOT_QUEST)) {
					for (int quest_stigma : elyosStigmaQuest) {
						completeQuest(player, quest_stigma);
					}
				}
			} else if (playerRace == Race.ASMODIANS) {
				switch (dialogId) {
				...
				if (player.havePermission(MembershipConfig.STIGMA_SLOT_QUEST)) {
					for (int quest_stigma : asmodianStigmaQuest) {
						completeQuest(player, quest_stigma);
					}
				}
			}
	...
	private static boolean validateSwitch(Player player, PlayerClass playerClass) {
		...
		int lvlToChange = GSConfig.STARTCLASS_MAXLEVEL - 1;
		int lvlToChangeMonk = GSConfig.STARTCLASS_MONK_MAXLEVEL - 1;
		PlayerClass oldClass = player.getPlayerClass();
		if (level != lvlToChange && oldClass != PlayerClass.MONK) {
			PacketSendUtility.sendMessage(player, "You can only switch class at level " + lvlToChange);
			return false;
		} 
		if (level != lvlToChangeMonk && oldClass == PlayerClass.MONK) {
			PacketSendUtility.sendMessage(player, "You can only switch class at level " + lvlToChangeMonk);
			return false;
	...
	public static void onUpdateMission1073(Player player) {
        if (player.getQuestStateList().hasQuest(1073)) {
			final QuestState qs = player.getQuestStateList().getQuestState(1073);
			if (qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 4) {
				qs.setQuestVar(4);
				qs.setStatus(QuestStatus.REWARD);
				PacketSendUtility.sendPacket(player, new S_QUEST(1073, qs.getStatus(), qs.getQuestVars().getQuestVars()));
			}
		}
    }
	public static void onUpdateFailMission1073(Player player) {
        if (player.getQuestStateList().hasQuest(1073)) {
			final QuestState qs = player.getQuestStateList().getQuestState(1073);
			if (qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 4) {
				qs.setQuestVar(3);
	...
	public static void onUpdateMission2073(Player player) {
        if (player.getQuestStateList().hasQuest(2073)) {
			final QuestState qs = player.getQuestStateList().getQuestState(2073);
			if (qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 4) {
				qs.setQuestVar(4);
				qs.setStatus(QuestStatus.REWARD);
				PacketSendUtility.sendPacket(player, new S_QUEST(2073, qs.getStatus(), qs.getQuestVars().getQuestVars()));
			}
		}
    }
	public static void onUpdateFailMission2073(Player player) {
        if (player.getQuestStateList().hasQuest(2073)) {
			final QuestState qs = player.getQuestStateList().getQuestState(2073);
			if (qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 4) {
				qs.setQuestVar(3);
