...
public abstract class CreatureController<T extends Creature> extends VisibleObjectController<Creature>
{
	...
	public void onRestore(HealType hopType, int value, int skillId) {
		switch (hopType) {
			case HP:
				getOwner().getLifeStats().increaseHp(TYPE.HP, value, skillId);
			break;
			case MP:
				getOwner().getLifeStats().increaseMp(TYPE.MP, value, skillId);
			break;
			case FP:
