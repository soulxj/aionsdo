...
public class PlayerController extends CreatureController<Player> {
	...
	public void onDie(Creature lastAttacker, boolean showPacket) {
		...
		if (AbyssService.isOnPvpMap(player) && ar != null) {
			...
			if (master != null && DuelService.getInstance().isDueling(player.getObjectId(), master.getObjectId())) {
				...
				final Player winner = (Player) master;
				winner.setInvul(true);
				winner.getLifeStats().setCurrentHp(player.getLifeStats().getMaxHp() / 3);
				...
				int []npcTraps = {749179, 749180, 749292, 749293, 749273, 
						749272, 749037, 749036, 749035, 749034, 296500, 296479, 
						296466, 749012, 749018, 749019, 749020, 74902, 749032,
						749033, 749240, 749241, 749143, 749144, 749145, 749146,
						749147, 749148, 749190, 749191, 749270, 749271, 749071,
						749072, 749073, 749074, 749077, 749078, 749079, 749080,
						749092, 749093, 749096, 749097, 749238, 749239, 749242, 749243,
						749277, 749276, 749269, 749268, 749237, 749236, 749184,
						749183, 749182, 749181, 749167, 749168, 749169, 749170,
						749171, 749172, 749196, 749197, 749290, 749291, 749296,
						749086, 749087, 749088, 749089, 749141, 749142, 749157,
						749158, 749192, 749193, 749234, 749235, 749274, 749275,
						749149, 749150,};
				int []npcServants = {833061, 833062, 833063, 833064, 833065, 833066, 833067, 833068,
						749217, 749218, 749219, 749220, 749221, 749222, 749223, 749224,
						833069, 833070, 833071, 833072, 833073, 833074, 833075, 833076,
						749225, 749226, 749227, 749228, 749229, 7492230, 7492231, 7492232};
				
				for (int npcTrap : npcTraps) {
					List<VisibleObject> traps = World.getInstance().getWorldMap(WorldMapType.SANCTUM.getId()).getMainWorldMapInstance().getNpcId(npcTrap);
					for (VisibleObject trap : traps) {
						Npc npc = (Npc) trap;
						if (npc.getCreator().getObjectId() == player.getObjectId()) {
							if (npc != null) {
				...
				for (int npcServant : npcServants) {
					List<VisibleObject> servants = World.getInstance().getWorldMap(WorldMapType.SANCTUM.getId()).getMainWorldMapInstance().getNpcId(npcServant);
					for (VisibleObject servant : servants) {
						Npc npc = (Npc) servant;
						if (npc.getCreator().getObjectId() == player.getObjectId()) {
							if (npc != null) {
								npc.getController().onDelete();
	...
	public void cancelCurrentSkill() {
		...
		if (player.getSkillNumber() > 0) {
			player.setSkillNumber(player.getSkillNumber() - 1);
		}
	...
	@Override
	public void onRestore(HealType healType, int value, int skillId) {
		super.onRestore(healType, value, skillId);
		switch (healType) {
			case DP:
